...
test('Codex native interrupt addresses the fresh active turn and waits until it settles', { skip: platform() !== 'linux' }, async () => {
  const previousHome = process.env.SPEXCODE_HOME
  const previousSocketDir = process.env.SPEXCODE_CODEX_SOCKET_DIR
  const home = mkdtempSync(join(tmpdir(), 'spex-codex-interrupt-'))
  process.env.SPEXCODE_HOME = home
  process.env.SPEXCODE_CODEX_SOCKET_DIR = join(home, 'sockets')
  const target = 'interrupt-target-thread'
  const turnId = 'interrupt-target-turn'
  let status = 'inProgress'
  const interrupts: unknown[] = []
  const server = codexRpcFixture((message) => {
    if (message.method === 'thread/turns/list') return { data: [{ id: turnId, status }], nextCursor: null }
  ...
  try {
    ...
    mkdirSync(root, { recursive: true })
    owner = startCodexOwner(root)
    assert.deepEqual(await codexHarness.interrupt?.({ session: 'interrupt-session', harnessSessionId: target, runtimeDir: root }), { ok: true })
    assert.deepEqual(interrupts, [{ threadId: target, turnId }])
  } finally {
    await new Promise<void>((resolve) => server.close(() => resolve()))
    await stopCodexOwner(owner)
    if (previousHome === undefined) delete process.env.SPEXCODE_HOME
...
test('Codex native interrupt treats an unmaterialized thread as having no turn', { skip: platform() !== 'linux' }, async () => {
  ...
  const interrupts: unknown[] = []
  const server = codexRpcFixture((message) => {
    if (message.method === 'thread/turns/list') throw new Error(`thread not loaded: ${target}`)
...
test('Codex cold archive accepts only an explicitly unmaterialized absent native thread', { skip: platform() !== 'linux' }, async () => {
  const previousHome = process.env.SPEXCODE_HOME
  const previousSocketDir = process.env.SPEXCODE_CODEX_SOCKET_DIR
  const home = mkdtempSync(join(tmpdir(), 'spex-codex-cold-unmaterialized-'))
  process.env.SPEXCODE_HOME = home
  process.env.SPEXCODE_CODEX_SOCKET_DIR = join(home, 'sockets')
  const target = 'unmaterialized-cold-target'
  ...
  const server = codexRpcFixture((message) => {
    ...
    if (message.method === 'thread/delete') { archiveCalls.push({ method: message.method, ...message.params }); loaded = false; return {} }
    if (message.method === 'thread/list') return { data: [], nextCursor: null }
    throw new Error(`unexpected RPC ${message.method}`)
  })
  const root = runtimeRoot()
  const socket = codexAppServerSock(root)
  ...
  try {
    ...
    assert.equal(preflight.alreadyCold, undefined)
    assert.deepEqual(await codexHarness.coldRuntime?.(rec, preflight.receipt), { ok: true })
    assert.deepEqual(archiveCalls, [{ method: 'thread/delete', threadId: target }], 'a loaded unmaterialized thread is removed by the exact native delete mutation')
  } finally {
    await new Promise<void>((resolve) => server.close(() => resolve()))
    await stopCodexOwner(owner)
    if (previousHome === undefined) delete process.env.SPEXCODE_HOME
...
test('codex resource probe reads status only for governed references', async () => {
  ...
  const previousSocketDir = process.env.SPEXCODE_CODEX_SOCKET_DIR
  process.env.SPEXCODE_CODEX_SOCKET_DIR = join(dir, 'sockets')
  const target = 'governed-thread'
  const unowned = 'unowned-history-thread'
  const reads: string[] = []
  const server = codexRpcFixture((message) => {
    if (message.method === 'thread/loaded/list') return { data: [{ id: target }, { id: unowned }], nextCursor: null }
    if (message.method === 'thread/read') {
  ...
  try {
    ...
    assert.deepEqual(result, {
      healthy: true,
      references: [
        { referenceId: target, turnPresence: 'idle' },
        { referenceId: unowned, turnPresence: 'unknown' },
      ],
    })
