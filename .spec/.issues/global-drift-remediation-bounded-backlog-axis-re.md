...
<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-10T15:57:57.933Z -->
Spec: event-detail\n\nLanded 297d7dd37, spec(event-detail): scope session history eval axis.\n\nScenario session-home-history-rooted now names EventDetail rather than the entire EventDetail.jsx file. Its existing EvalsPage selectors continue to own the session-scoped projection and bounded-history fetch. EventDetail owns only the supplied-history handoff, viewed reading selection, and A/B cursor/navigation.\n\nNo product source, scenario prose or expectation, freshness rule, gate, or score changed. The scenario remains stale because its historical anchor is gone and its code axis changed; no reading was refiled.\n\nPost-landing verification on main: spex spec lint = 0 errors / 42 warnings; spex eval lint = 138 flagged (0 malformed, 599 stale, 0 missing, 0 coverage gaps, 37 over-owned). EventDetail.jsx ownership is 14, down from 15.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-10T23:53:51.111Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re\n\nEvidence-audit checkpoint from ZSwarm act1 r3, reported by session 59234d18-3c3a-4632-bbcf-845685a8ea54.\n\nThe functional evidence is retained: a real user project started with zero spec nodes, ordinary HTTP observed the graph grow to 26 nodes, the node paths identify the user module rather than ZCode, and the sole starting Git commit contained no .spec entries. This is not being recast as a product failure.\n\nFrame review found an omitted delivery fact: the recording visibly ran spex init with the opencode harness, even though the work was performed by ZCode swarm children and the requested demo must use the zcode harness. A one-flag r4 re-record is in progress; r3 remains preserved evidence and the defect remains disclosed.\n\nGovernance rule: for a video acceptance claim, structured counters and logs prove that an event occurred, but sampled rendered frames prove what product, identity, and interaction actually appeared. A report can be factually accurate yet omit visible delivery facts. Video review therefore requires both same-surface measurement and frame inspection; neither substitutes for the other. No freshness rule, gate, product source, or evaluation schema changed.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-10T23:54:55.092Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re\n\nOperational refinement for the video-evidence rule.\n\nVideo acceptance frame review must cover three concrete facts, with at least one frame from each of the opening state, the claimed transition outcome, and the ending state:\n\n1. Product identity. Inspect the rendered title, URL, or application shell to establish which product is actually on screen, rather than relying on the report label.\n2. Tool identity. Inspect readable commands and configuration in the rendered pixels to establish which tool or harness is attached to the workspace. This fact may appear in no counter, JSON result, or gate; the act1 r3 visible harness flag was the motivating example.\n3. State transition. Inspect a real before frame and a real after frame for the claimed change. An after frame alone is insufficient. Prefer a product-rendered empty-state explanation when available: it proves that the product itself recognized the starting absence, stronger than an external zero count because it rules out a broken count pipeline.\n\nThe rule remains: counters prove occurrence; frames prove the product, tool, and rendered before/after meaning of that occurrence. r3 is retained as flow evidence; r4 corrects only the harness presentation and does not erase r3 or its disclosed mismatch. No product source, freshness criterion, gate, or evaluation schema changed.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T00:42:18.430Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re\n\nVideo evidence checkpoint from ZSwarm act1 r4, reported by session 59234d18-3c3a-4632-bbcf-845685a8ea54.\n\nr4 is accepted as an honest record: the rendered ZCode product and user project identity match; visible initialization uses the zcode harness; the opening product self-declares no spec tree and 0 live nodes; the transition ends with a rendered 26-node tree; Git history proves the starting commit held neither .spec nor .opencode. Ordinary HTTP observed the first graph growth after 67.782 seconds. No retry hides the earlier r3 harness mismatch.\n\nA composition audit found a separate product fact. Of the 26 visible graph nodes, 4 represent the user project and 22 are project/.plugins scaffold nodes. The total is true, the graph is real, and the tree did grow, but the headline does not by itself mean that 26 user-project units were discovered. The correct disposition is not to hide scaffolding or re-record with a smaller preset. It is a ZCode feature lane: make the first graph view prioritize the user project while retaining an accessible scaffold view. The required negative check is that scaffolding remains reachable; a solution that merely suppresses it fails that check.\n\nEvidence review has three distinct layers: occurrence (counters, logs, gates); rendered identity and interaction (frames); and semantic composition of the reported values. Each can pass while the next fails. For video delivery, all three are required before claiming that the displayed metric means what its label implies. No SpexCode product source, freshness criterion, gate, score, or evaluation schema changed.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T00:43:17.250Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re\n\nBounded trigger for the third video-evidence review layer.\n\nDo not decompose every reported number. Decompose a number only when it is used to support a claim about scope or scale. The question is what claim the number is underwriting, not whether the number is large.\n\nExample: 26 live nodes was used to support the scale claim that a project graph grew from zero. Its composition therefore had to be inspected: 4 user-project nodes plus 22 project/.plugins scaffold nodes. The transition claim remains true, but the headline overstates user-project scope.\n\nCounterexample: first visibility at 67.782 seconds is a timing claim, not a scope/scale claim. Decomposing that duration has no expected information gain and is not required by this rule.\n\nThis makes the third layer executable and bounded. The existing ZCode follow-up retains its negative check: a user-project-first view is valid only if the scaffold remains reachable, so a hide-the-plugins-only change cannot satisfy the acceptance case. No product source, freshness criterion, gate, score, or evaluation schema changed.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T01:04:40.268Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re\n\nCorrection: distinguish proven method lessons from unfinished feature delivery. The following are observed facts from the ZSwarm governance-grouping lane, not pending hypotheses.\n\n1. A request that names multiple product surfaces can misdirect the implementation. The task wording placed SpexCode spec-dashboard and the ZCode UI side by side. The lane changed Dashboard.jsx, GraphStats.jsx, and SpecNode.jsx, while the recorded act1 target was ZCode packages/ui/src/swarm-map/SpecMap.tsx. The dashboard result may be useful, but it was not the requested product surface. State the target product and surface explicitly; list other surfaces only as separate follow-ups.\n\n2. A claimed existing baseline failure must be reproduced on the latest shared integration head before being recorded as a blocker. The claimed reviewQuery/reviewFilters failure was directly falsified: clean main passed 6 of 6, and the lane worktree also passed 6 of 6. The unrepeatable report is residue, not baseline truth.\n\n3. A prohibition must name its exact scope. A ban on commands that write refs or stash was generalized to a ban on any ref write, so the lane withheld its required commit and therefore could not file an eval reading with a real codeSha. Constraints must distinguish prohibited side effects from required normal workflow actions.\n\nFeature delivery is separately unfinished: the dashboard change awaits its commit, codeSha-bound reading, and browser proof; ZCode SpecMap needs its own lane. This status does not downgrade the three observed lessons above. No SpexCode product source, freshness criterion, gate, score, or evaluation schema changed.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T01:32:15.535Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re\n\nConstraint-effectiveness evidence checkpoint from the ZCode SpecMap target-surface lane.\n\nA constraint is not automatically proved useful because a child did not violate it. Grade the constraint itself by evidence:\n\n- Confirmed effective requires a comparable prior failure and a later run in the same decision point that avoids it because the constraint is explicit. Here, the scoped prohibition is confirmed: an earlier lane generalized a ban on ref or stash writes into no ref writes and withheld its required commit; the revised instruction explicitly allowed and required the node-branch commit, and the ZCode lane committed normally.\n- Untested means the run presented no observable chance to distinguish the constraint from ordinary correct execution. The single-target-surface instruction and shared-head baseline-reproduction instruction were not triggered because this lane neither edited another surface nor reported a baseline failure. They are not credited as successes, nor inferred decorative, and must not be exercised by manufacturing violations.\n\nThe same lane provides a product-level example of a negative acceptance check: ZCode SpecMap now presents 4 project and 22 SpexCode nodes in a collapsed governance group, while an interactive browser test expands the group and proves every one of the 22 governance identifiers renders exactly once. Exactly once excludes both suppression and duplicate counting. Its classification must rely on graph ancestry rather than short node names because the client receives no path metadata and user modules can share names such as core or commands. The ZCode lane remains responsible for documenting the stable governance-root criterion before merge. No SpexCode product source, freshness criterion, gate, score, or evaluation schema changed.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T01:34:03.478Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re

Cross-product handoff status, not a SpexCode source change: both Act 1 spec-map governance surfaces have independently passed real-browser evidence. The SpexCode dashboard proposal is f789a8519 plus evidence commit 1899d9ab9; the ZCode SpecMap proposal is 9b44fb148. In each, the initial view prioritizes four user-project nodes while 22 SpexCode scaffold nodes remain reachable by expanding the governance group. The reachability proof is intentionally the check that a hide-the-scaffold implementation cannot pass.

ZCode proposal remains awaiting human merge. Its lane must retain one merge condition: document in an owning spec or a local navigation comment why governance classification follows the graph parent chain instead of short id names. The client removes path metadata, so name matching could misclassify user modules called core or commands. The lane must also state what identifies the governance root and whether that identifier is a stable contract. These are maintainability facts, not a request to change drift, freshness, gates, scores, schemas, or core SpexCode behavior.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T01:40:21.973Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re

Cross-repository delivery correction, no source change: two ZCode deliveries reported real output on external commits while their SpexCode node branches were clean and zero commits ahead. This is evidence to retain, but the claimed forced choice between an empty SpexCode commit and propose-close is incomplete.

Current lifecycle truth is already narrower. A merge proposal claims local committed work to land, so the stop hook correctly blocks dirty or zero-ahead merge. A close proposal is explicitly exempt, but is truthful only after the external delivery has landed and there is no pending human review, decision, or artifact inspection. When an external commit or proposal still awaits a human merge, the truthful existing declaration is asking from the original SpexCode session worktree, with a note naming the external repository, immutable commit, and awaited action. Asking does not enter the commit gate. Running a declaration from the external repository reports no session record because session stores are project-scoped; that is a wrong-store diagnosis, not evidence that the lifecycle lacks a state.

Operational rule: never create an empty SpexCode commit to satisfy a declaration gate. Before external landing, declare asking in the source session's project. After verified external landing and no remaining human action, declare close-pending there. The broader deliver-port node is an explicitly pending design; do not revive or widen it through this incident without a separate product decision. This records the evidence and removes the false dilemma without changing drift, freshness, gates, scores, schemas, or session semantics.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T01:42:34.900Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re

Correction to the preceding cross-repository delivery record: the statement that the commit gate forced an empty SpexCode commit or propose-close was too broad. The current stop contract already has two truthful exits. Before an external result lands, while a human merge, review, inspection, or other decision remains, the source SpexCode session must declare asking from its own project worktree and name the external immutable commit plus awaited action. After the external result has landed and no human decision or follow-up remains, close-pending is truthful and is explicitly exempt from the commit check.

Therefore previous external sessions which used close while their ZCode proposals still awaited a human merge are historical state errors, not evidence of a lifecycle-schema defect; their owners are correcting those records. The retained product improvement candidate is only stop-gate wording: a zero-ahead merge refusal currently tells an agent to commit local spec and code, but does not name the existing asking route for a cross-repository handoff. Keep that as a narrow discoverability improvement candidate, not a basis for changing proposal types, commit-gate behavior, drift, freshness, scores, schemas, or the pending deliver-port design.

<!-- reply: fbb76f84-7a73-4262-81d6-9028f5eb7c4e @ 2026-08-11T01:43:32.360Z -->
Spec: global-drift-remediation-bounded-backlog-axis-re
