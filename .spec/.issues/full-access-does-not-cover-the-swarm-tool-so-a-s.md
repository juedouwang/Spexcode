...
<!-- reply: 644c22c2-e6db-427f-aa24-3a2d883c0336 @ 2026-08-15T04:49:32.525Z -->
# 重述附注补遗 · 第三条：手里是观测，嘴里说的是机制

前一条附注讲截断器。这一条讲同一族里更隐蔽的一半：**读数是真的，但把它翻译成机制时多加了一个假设，
而那个假设从没被验过。**

本次排查里的实例（未造成后果，因为在发出前自查住了）：

读这份 issue 文件时，我看到 `<!--​reply: <session-id> @ <时刻> -->` 标记，
默认它是**块尾签名**。按这个假设读下去，会得出「有一条署名为我的回复，其内容却是写给我的」——
即署名错乱，看起来是一个值得上报的数据完整性缺陷。

实际标记在**块首**。核法很便宜：找第一条标记，看紧随其后的那段是不是我写的——是。
一个假设、一条命令，结论完全反过来。

同族的第三个实例（同一次排查内）：判据的两个分支都在整页 `innerText` 上做正则，
读数每次都是真的，但把「页面里出现了 Awaiting approval」翻译成「这次调用停在审批上」时，
默认了「这段文字只可能来自当前任务」——而侧栏挂着别的工作区的任务标题。
同一自变量下两次相反判定，就是这个多余假设的账单。

**规则**：报告一个机制之前，先把它依赖的那个隐含假设单独写出来，并给出**一条能推翻它的命令**。
写不出那条命令，就只报观测，不报机制。

<!-- reply: 644c22c2-e6db-427f-aa24-3a2d883c0336 @ 2026-08-15T05:51:16.517Z -->
# 附注补遗 · 第四条：判据与动作不许同乘一条命令

前两条附注讲截断器、讲"手里是观测嘴里说的是机制"。这一条把它们的共同根挖出来，
因为一天之内它长出了**四个**不同模样的实例。

## 四个实例

    ① grep … | head -6         命中多于 6 条，拿被截断的输出下了"这棵树上不存在"的断言
    ② systemd-run … ; echo "EXIT=$?"   最后一条是 echo，构建的 exit 2 被报成 exit 0
    ③ npx tsc … ; … && git commit      ';' 断开了 '&&'，tsc 失败而提交照常发生
    ④ git bundle create <新> && rm -f <旧>   新包名取自一个"本以为会变、实际没变"的 HEAD，
                                             于是 rm 删掉的正是刚建好的那一份

前三个是同一个人当天犯的，第四个是另一个人。**所以这不是手滑，是形状。**

## 共同形状

不是"用错了 shell 语法"，是：
**把要检查的东西和要执行的东西放进同一条命令，然后让最后一个成功者代表整条命令。**

一条命令只有一个退出码、一份输出。把判据和动作串在一起，等于让动作去回答判据的问题。

## 两条规矩

    ① 判据命令必须以被判据的那条命令结尾；
       它的退出码与输出都不许被后续命令（echo / head / tail / 管道 / 另一条 &&）覆盖。
    ② 不可逆动作（删除、覆盖、提交、推送）必须与产出动作分开执行，
       并且排在产出被验证之后。

## 它在产品里的同一形状

这条不只长在取证里。同期在交付闸上量到：lint 的失败理由被 `summarize()` 截到 400 字符且只保头，
而 lint 把"怎么修"放在末尾——worker 收到的失败理由里，可执行的那半恰好被切掉。
规则 ① 的产品版本就是：**一条要被别人据以行动的消息，不许让截断决定它保留哪一半。**
