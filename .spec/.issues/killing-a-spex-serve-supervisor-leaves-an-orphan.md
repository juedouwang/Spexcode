...
closedAt: 2026-09-23T20:33:37.713Z
---
...
<!-- reply: 0edd38cf-8197-44c6-876d-b63410c7ee4f @ 2026-08-06T11:23:17.035Z -->
⚠ **紧急更正上一条:我给出的那条「安全删除判据」有一条腿是死的,照抄会删掉正在使用的夹具。**

我写的是三信号合取(无 cwd / 无 fd / 近 N 分钟无写),并报「181 全部判死」。**mtime 那条腿从未真正执行过。**

```
本机 find 实为 bfs;bfs 拒绝相对日期串:
  find /tmp -maxdepth 0 -newermt '-60 min'
  -> bfs: error: ... -newermt "-60 min"        ← 直接报错,零输出
而我把 stderr 重定向进了 /dev/null,于是"报错"与"没有近期写入"在我的脚本里长得一模一样,
空输出被判成"不是最近写的" ⇒ 全部 181 落进"可安全删"。
```

用能工作的写法(`-mmin -60`)重算:
```
进程占用(cwd 或 fd)        ->   0
60 分钟内目录内有写         ->   6      ← 原本被吞掉的那一格
三条判据都判死(可安全删)  -> 175      ← 不是 181
```

**⇒ 上一条里那句「181 全部判死」和据此写出的删除配方作废。** 正确判据是三条合取,且 mtime 腿必须用
`-mmin`(或明确时刻串)而不是相对日期串,并且**不得吞掉 stderr**。

**这次翻车本身是本 issue 主题的同构实例,值得记进来:**
本 issue 讲的是「沉默的正常态与故障不可区分」。**我为了排查它而写的检测脚本,自己犯了同一个病** ——
`2>/dev/null` 把一次**执行失败**转成了一个**看起来干净的结果**。
⇒ **凡是「以输出为空作为判据」的测量命令,绝不能吞 stderr:失败的空与真实的空,在下游眼里完全一样。**
