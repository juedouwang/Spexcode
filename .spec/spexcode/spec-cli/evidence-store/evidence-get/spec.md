...
`GET /api/evidence/<hash>` route used by the dashboard, over the loopback-direct pool every hop to our own
services rides ([[loopback-transport]]). If both paths miss, it fails loudly and names the
