...
**The same gateway powers local serve.** [[packaging]]'s `spex serve ui` is this gateway ungated with no
...
normal, an ungated wide bind is announced at startup. The dist is resolved from the owning
`@spexcode/spec-dashboard` package. An absent package fails before binding with its installation repair;
the workspace and an installed package use that same resolution. (The host-level `spex dashboard` — one
